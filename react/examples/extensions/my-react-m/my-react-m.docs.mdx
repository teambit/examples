...
Under the **variant** section of your `workspace.json` file choose which components you want to have the custom environment set. You can find the id of the extension in the `my-react-m.aspect.ts` file.

```json
{
  "teambit.workspace/variants": {
    "{ui/**}, {pages/**}": {
      "react/examples/extensions/my-react-m": {}
    }
  }
}
```
...
Bit uses ESLint to lint your components. You can add your own rules in the `my-react-m.main.runtime` file.
...
Bit uses Prettier to format your components. You can add your own rules in the `my-react-m.main.runtime` file.
...
Override the default dependencies in the `my-react-m.main.runtime` file to include react types of a different version for example.
...
Extend the `my-react-m.preview.runtime` file when you want to add your own customizations only for previewing in the Bit UI.
